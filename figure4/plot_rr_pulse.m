...
load('pulse_rr_rho4.mat','RR','RR_inf','list_kappa','list_theta')
...
%     opt_RR(k) = R_kappa(pos(1))
end

M = movmean(opt_theta,5);
...
%%HH 
load('asym/asHHr4_pulse.mat','RR','list_kappa','list_theta')
...
%%HL
load('asym/asHLr4_pulse.mat','RR','list_kappa','list_theta')
[row, col] = find(ismember(RR, max(RR(:))));
...
%%LH
load('asym/asLHr4_pulse.mat','RR','list_kappa','list_theta')
[row, col] = find(ismember(RR, max(RR(:))));
...
%%LL
load('asym/asLLr4_pulse.mat','RR','list_kappa','list_theta')
[row, col] = find(ismember(RR, max(RR(:))));
...
%%sym
load('pulse_rr_rho4.mat','RR','list_kappa','list_theta')
[row, col] = find(ismember(RR, max(RR(:))));
...
t_H = -5;   t_L = -0.5;  k_H = 10;   k_L = 0.5;
rr_val = [max_RR_HH max_RR_HL max_RR_LH max_RR_LL]% max_RR]
bx = [k_H opt_kappa_HH ;k_L opt_kappa_HL ;k_H opt_kappa_LH ;k_L opt_kappa_LL ]% opt_kappa]
by = [t_H opt_theta_HH ;t_H opt_theta_HL ;t_L opt_theta_LH ;t_L opt_theta_LL ]% opt_theta]
ms = abs([MS_HH MS_HL MS_LH MS_LL])./abs(MS)
...
b.CData = Bl(11,:);
set(gca,'fontsize',30,'xticklabel',{[]}); box off; xlim([0.5 1.5]); ylim([max_RR_HL max_RR]);
set(groot,'defaultAxesTickLabelInterpreter','latex');
...
plot(linspace(.5,1.5,4),max_RR*ones(1,4),'Color',[.4 .4 .4])%,'BarWidth',.25);
b.CData = Pr(11,:);
set(gca,'fontsize',30,'xticklabel',{[]},'yticklabel',{[]}); box off; xlim([0.5 1.5]); ylim([max_RR_HL max_RR]);
...
load('pulse_rr_rho4.mat','RR','list_kappa','list_theta','av_r','av_T1','av_T2','p_1','p_2','t','rho')
[row, col] = find(ismember(RR, max(RR(:))));
p_1 = squeeze(p_1(row(1),col(1),:)); p_1 = [0; p_1];
p_2 = squeeze(p_2(row(1),col(1),:)); p_2 = [0; p_2];
plot(t,p_1(1:end-1),'LineWidth',3,'color',[0 0 0])
set(gca,'fontsize',30); 
...
%%HH
load('asym/asHHr4_pulse.mat','RR','p_1','p_2','t')
[row, col] = find(ismember(RR, max(RR(:))));
...
%%LH
load('asym/asLHr4_pulse.mat','RR','p_1','p_2','t')
[row, col] = find(ismember(RR, max(RR(:))));
p_1 = squeeze(p_1(row(1),col(1),:)); p_1 = [0; p_1]; p_1 = p_1(1:end-1);
p_2 = squeeze(p_2(row(1),col(1),:)); p_2 = [0; p_2]; p_2 = p_2(1:end-1);
plot(t,p_1,'LineWidth',3,'color',Rd(11,:))
% legend('$\theta_1 = \theta_2$, $\kappa_1 = \kappa_2$','$\theta_1 = -5$, $\kappa_1 = 10$','$\theta_1 = -.5$, $\kappa_1 = 10$','Interpreter','latex','location','West','EdgeColor','none')

%%HL
ax2 = axes('Parent',f,'Units','normalized','Position',[0.425,0.188405797101449,0.251579925650558,0.737784679089028]);
hold on;
load('asym/asHLr4_pulse.mat','RR','p_1','p_2','t')
[row, col] = find(ismember(RR, max(RR(:))));
p_1 = squeeze(p_1(row(1),col(1),:)); p_1 = [0; p_1]; p_1 = p_1(1:end-1);
p_2 = squeeze(p_2(row(1),col(1),:)); p_2 = [0; p_2]; p_2 = p_2(1:end-1);
...
%%LL
ax3 = axes('Parent',f);
set(ax3,'Units','normalized','Position',[0.684014869888476,0.188405797101449,0.261107081331036,0.740165631469982]);
hold on;
load('asym/asLLr4_pulse.mat','RR','p_1','p_2','t')
[row, col] = find(ismember(RR, max(RR(:))));
p_1 = squeeze(p_1(row(1),col(1),:)); p_1 = [0; p_1]; p_1 = p_1(1:end-1);
p_2 = squeeze(p_2(row(1),col(1),:)); p_2 = [0; p_2]; p_2 = p_2(1:end-1);
